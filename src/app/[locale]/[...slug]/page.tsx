...
export default async function CatchAllPage({ params }: { params: Promise<{ locale: string; slug: string[] }> }) {
  ...
  if (isContentType(first)) {
    ...
    const GUIDE_STOP_WORDS = new Set(['phantom', 'blade', 'zero', 'soul', 's-game', 'kungfupunk', 'wuxia', 'sha-chi', 'ghostep', 'hellwalker', 'breached', 'parry', 'dodge', 'combat', 'guide', 'the', 'a', 'an', 'and', 'or', 'in', 'on', 'at', 'to', 'for', 'is', 'it', 'how', 'what', 'all', 'vs', 'game', 'wiki', 'best', 'build', 'tips', 'tier', 'list', 'rank', 'weapon', 'boss', 'ending', 'edge', 'update', 'lore', 'difficulty']);
