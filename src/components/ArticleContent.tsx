  ...
  useEffect(() => {
    ...
    if (locale === 'en') {
      ...
      anchors.forEach((a) => {
        const href = a.getAttribute('href');
        if (!href) return;
        if (href.startsWith(localePrefix) || href.startsWith(`/${locale}`)) return;
        if (href.startsWith('/en/')) {
          a.setAttribute('href', `/${locale}${href.slice(3)}`);
        } else if (!href.startsWith('/')) {
          // external or non-root paths — skip
        } else {
          const trailing = href.endsWith('/') ? '' : '/';
          a.setAttribute('href', `/${locale}${href}${trailing}`);
        }
      });
    }
  }, [locale]);
